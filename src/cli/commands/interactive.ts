...
function renderWelcome(): void {
    ...
    const logo = [
        ...
        `   ${chalk.hex(ACCENT2)("›")} ${chalk.bold("probe")} <url>                       ${chalk.dim("monitorar (use --save <nome>.json)")}`,
...
function renderHelp(): void {
    const help = [
        ...
        "",
        `   ${chalk.hex(ACCENT2).bold("Monitoramento e Relatórios:")}`,
        ...
        `   ${chalk.bold("probe <url> --save <f>")}  Salva sessão em JSON para gerar relatórios`,
        `   ${chalk.bold("report -i <f> -f html")}    (Use FORA daqui) Gera relatório visual`,
        "",
